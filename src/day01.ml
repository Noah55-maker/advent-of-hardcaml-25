...
module O = struct
  type 'a t =
    { (* With_valid.t is an Interface type that contains a [valid] and a [value] field. *)
      answer : 'a With_valid.t [@bits num_bits]
    }
  [@@deriving hardcaml]
end
...
let create scope ({ clock; clear; start; finish; data_in; data_in_valid } : _ I.t) : _ O.t
  ...
  let%hw_var dir = Variable.reg spec ~width:1 in (* 0:left, 1:right *)

  (* FIFO *)
  let%hw_var fifo_rd = Variable.wire ~default:gnd () in
  let%tydi { q = fifo_front; full = fifo_full; empty = fifo_empty; _ } = Fifo.create
      ~showahead:true ~scope:(Scope.sub_scope scope "fifo")
      ~capacity:5000 ~overflow_check:true ~underflow_check:true
      ~clock ~clear
      ~wr:data_in_valid ~d:data_in ~rd:fifo_rd.value () in
  let%hw fifo_full in
  ...
  let answer = Variable.wire ~default:(zero num_bits) () in
  let answer_valid = Variable.wire ~default:gnd () in
  compile
    [ sm.switch
        [ ( Idle
          , [ when_
        ...
        ; ( Accepting_inputs
          , [ when_ data_in_valid
              [ when_ (fifo_full) [ sm.set_next Looping ] ]
            ; when_ finish [ sm.set_next Looping ]
            ] )
        ; ( Looping
            , [ if_ (fifo_empty &: (clicks.value ==:. 0)) [ sm.set_next Done ]
                @@ elif (clicks.value ==:. 0) (* Read next input from FIFO *)
                    [ when_ (dial.value ==:. 0) [ count0 <-- count0.value +:. 1 ]
